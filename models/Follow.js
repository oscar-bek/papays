...
class Follow {
	...
	async subscribeData(member, data) {
		try {
			...
			const result = await this.createSubscribtionData(follow_id, subscriber_id);
	...
	async modifyMemberFollowCounts(mb_id, type, modifier) {
		try {
			if (type === 'follow_change') {
				await this.memberModel.findByIdAndUpdate({ _id: mb_id }, { $inc: { mb_follow_cnt: modifier } }).exec();
			} else if (type === 'subscriber_change') {
				await this.memberModel.findByIdAndUpdate({ _id: mb_id }, { $inc: { mb_subscriber_cnt: modifier } }).exec();
			}
		} catch (err) {
			throw err;
		}
