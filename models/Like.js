...
class Like {
	...
	async validateTargetItem(id, group_type) {
		try {
			...
			switch (group_type) {
				case 'member':
					result = await this.memberModel.findOne({ _id: id, mb_status: 'ACTIVE' }).exec();
					break;
				case 'product':
					result = await this.productModel.findOne({ _id: id, product_status: 'PROCESS' }).exec();
					break;
				case 'community':
				default:
					result = await this.boArticleModel.findOne({ _id: id, art_status: 'active' }).exec();
					break;
			}
			console.log(result);
			return !!result;
